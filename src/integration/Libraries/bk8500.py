...
def set_remote(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x20
    if (value):
        ...
        cmd[3] = 0x00
    if verbose == True: print("Setting instrument to remote = " + str(bool(value)))
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)


# enable(True)/disable(False) input current on the load
# gets serial port object, instrument address, and True/False as args
def enable_input(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x21
    if (value):
        ...
        cmd[3] = 0x00
    if verbose == True: print("Setting instrument input on = " + str(bool(value)))
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
def set_max_voltage(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x22
    cmd[3:7] = prep_cmd_volt_res_pow(value)
    if verbose == True: print("Setting instrument max voltage = " + str(value) + "[V]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
# gets serial port object, instrument address, and desired value as args
def set_max_current(ser,address,value, verbose):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x24
    cmd[3:7] = prep_cmd_cur(value)
    if verbose == True: print("Setting instrument max current = " + str(value) + "[A]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
# gets serial port object, instrument address, and desired value as args
def set_max_power(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x26
    cmd[3:7] = prep_cmd_volt_res_pow(value)
    if verbose == True: print("Setting instrument max power = " + str(value) + "[W]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
def set_mode(ser,address,mode, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x28
    if (mode == "CC"):
        ...
        return
    if verbose == True : print("Setting instrument mode = " + mode)
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
# gets serial port object, instrument address, and desired value as args
def set_current(ser,address,value, verbose):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x2a
    cmd[3:7] = prep_cmd_cur(value)
    if verbose == True : print("Setting instrument current = " + str(value) + "[A]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
# gets serial port object, instrument address, and desired value as args
def set_voltage(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x2c
    cmd[3:7] = prep_cmd_volt_res_pow(value)
    if verbose == True :print("Setting instrument voltage = " + str(value) + "[V]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
...
# gets serial port object, instrument address, and desired value as args
def set_resistance(ser,address,value, verbose = False):
    cmd = empty_cmd
    cmd[1] = address
    cmd[2] = 0x30
    cmd[3:7] = prep_cmd_volt_res_pow(value)
    if verbose == True :print("Setting instrument resistance = " + str(value) + "[Ohm]")
    ser.write(modify_checksum(cmd))
    if verbose == True: return get_response(ser)
